...
test('creates completed and history messages', () => {
  const completed = createJourneyCompletedMessage(journey);
  assert.equal(completed.text.includes('已收進你的咖啡足跡'), true);
  assert.deepEqual(
    (completed.quickReply?.items ?? []).map((item) => item.action?.type),
    ['message', 'message']
  );
