...
test('offers the cafe passport from the welcome message', () => {
  const message = createWelcomeMessage();
  assert.match(message.text, /我的咖啡護照/);
  assert.equal(
    (message.quickReply?.items ?? []).some(
      (item) => item.action?.type === 'message' && item.action.text === '我的咖啡護照'
    ),
    true
  );
});
