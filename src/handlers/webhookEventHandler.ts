...
import { handleMenuImageEvent } from './menuImageHandler.js';
import {
...
import { handleGroupScheduleText } from './groupScheduleTextHandler.js';
import { handlePassportText } from './passportTextHandler.js';
...
export async function handleWebhookEvent(event: WebhookEvent): Promise<void> {
  ...
  if (event.type === 'join' && event.source.type === 'group') {
    await reply(event.replyToken, [createGroupJoinMessage()]);
    return;
  }
  ...
  if (event.message.type === 'text') {
    if (isMenuScanCommand(event.message.text)) {
      await reply(event.replyToken, [createMenuScanGuideMessage()]);
      return;
    }

    const ownerId = getActorId(event.source) ?? '';
    const groupScheduleMessages = await handleGroupScheduleText({
      actorId: ownerId,
      conversationId: getConversationId(event.source) ?? '',
      sourceType: event.source.type,
      text: event.message.text
    });
    if (groupScheduleMessages) {
      await reply(event.replyToken, groupScheduleMessages);
      return;
    }
    const groupPlannerMessages = await handleGroupPlannerText({
      actorId: ownerId,
      conversationId: getConversationId(event.source) ?? '',
      sourceType: event.source.type,
      text: event.message.text
    });
    if (groupPlannerMessages) {
      await reply(event.replyToken, groupPlannerMessages);
      return;
    }
    const passportMessages = await handlePassportText({
      ownerId,
      sourceType: event.source.type,
      text: event.message.text
    });
    if (passportMessages) {
      await reply(event.replyToken, passportMessages);
      return;
    }
  ...
  if (event.message.type === 'image') {
    await handleMenuImageEvent(event);
    return;
  }
